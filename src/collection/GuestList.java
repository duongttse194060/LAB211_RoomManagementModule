...
public class GuestList {
    ...
    public static void searchGuestById() {
        ...
        while (true) {
            Guest foundGuest = null;
            while (true) {
                System.out.println("Enter National ID to search guest: ");
                String id = sc.nextLine();
                foundGuest = searchById(id);
                if (foundGuest == null) {
                    System.out.println("No guest found with the requested ID " + "'" + id + "'");
                } else {
                    System.out.println("1 guest has been found! Display guest info...");
                    Displayer.displayGuestInformation(id);
                    break;
                }
            }
            while (true) {
                System.out.println("Do you want to search another guest? (Y/N): ");
                String choice = sc.nextLine();
                if (choice.equalsIgnoreCase("n")) {
    ...
    public static void removeGuestBooking() {
        Scanner sc = new Scanner(System.in);
        while (true) {
            Guest foundGuest = null;
            while (true) {
                System.out.println("Enter Guest National ID to cancel booking: ");
                String id = sc.nextLine();
                foundGuest = searchById(id);
                if (foundGuest == null) {
                    System.out.println("Booking details for ID '" + id + "' could not be found.");
                    break;
                } else {
                    System.out.println("1 guest has been found! ");
                    if (Inputter.checkDateInPast(foundGuest.getStartDate())) {
                        System.out.println("The room booking for this guest cannot be cancelled! ");
                    } else {
                        Displayer.displayGuestInformation(id);
                        System.out.println("Do you really want to cancel this guest's room booking? (Y/N):");
                        String choice = sc.nextLine();
                        if (choice.equalsIgnoreCase("n")) {
                            System.out.println("Keeping guest booking...");
                            break;
                        } else if (choice.equalsIgnoreCase("y")) {
                            System.out.println("“The booking associated with ID" + id + " has been successfully canceled.”");
            ...
            while (true) {
                System.out.println("Do you wish to cancel another booking? (Y/N)");
                String choice = sc.nextLine();
                if (choice.equalsIgnoreCase("n")) {
                    System.out.println("Back to the main menu...");
                    return;
                } else if (choice.equalsIgnoreCase("y")) {
                    System.out.println("Continue cancel booking...");
                    break;
                } else {
                    System.out.println("Invalid input. Please try again.");
                }
